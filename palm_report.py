...
def generate_free_report(image_bytes: bytes) -> dict:
    try:
        ...
        prompt = f"""以下是对用户手掌图片的观察记录：

{image_description}

你是一位融合东方玄学与现代心理洞察的掌纹解读师。
请基于上方的掌纹观察，用"玄学感知 + 逻辑依据"的风格生成三个模块的解读。

写作要求：
- 每个模块开头必须有一句"洞察型判断"，像在点出用户一个内在矛盾或隐藏特质
- 使用"你的手相里…"、"从掌纹来看…"、"这条纹路透露出…"等表达
- 不要使用专业术语（不要写智慧线、生命线、感情线等），用"掌纹特征"或"这条纹路"代替
- 每段结构：洞察 → 简短依据（来自掌纹观察）→ 延伸到生活场景
- 适当加入"不是所有人都会这样"的暗示，让用户感到被精准看见
- 语言有神秘感但不胡说，每一句都能对应掌纹观察中的具体特征
- conclusion 是一句有力的洞察结论，不超过15字
- interpretation 是完整的解读段落，3-4句，必须基于上方掌纹观察

如果掌纹观察中细节不足，在 interpretation 开头写"图片细节有限，以下解读仅供参考"。

输出格式（严格按此 JSON 格式）：
{{
  "thinking_style": {{
    "conclusion": "15字以内的洞察结论",
    "interpretation": "洞察开头句。依据掌纹特征的解释。延伸到生活的描述。暗示句。"
  }},
...
你是一位融合东方玄学与现代心理洞察的掌纹解读师。
请基于上方的掌纹观察，用"玄学感知 + 逻辑依据"的风格生成五个模块的深度解读。

写作要求：
- 每个模块开头必须有一句"洞察型判断"，像在点出用户一个内在矛盾或隐藏特质
- 使用"你的手相里…"、"从掌纹来看…"、"这条纹路透露出…"等表达
- 不要使用专业术语（不要写智慧线、生命线、感情线、命运线等），用"掌纹特征"或"这条纹路"代替
- 每段 analysis 结构：洞察 → 简短依据（来自掌纹观察）→ 延伸到生活场景
- 适当加入"不是所有人都会这样"的暗示，让用户感到被精准看见
- 语言有神秘感但不胡说，每一句都能对应掌纹观察中的具体特征
- blind_spots 每条是一个具体的生活场景盲区，用 · 分隔，不超过20字/条
- suggestions 每条是可立即执行的具体行动，用 · 分隔，不超过25字/条
- reminder 是一句有力的金句，带一点玄学意味但落地
- conclusion 不超过15字，是整个模块最核心的洞察

如果掌纹观察中细节不足，在 analysis 开头写"图片细节有限，以下解读仅供参考"。

输出格式（严格按此 JSON 格式）：
{{
  "decision_structure": {{
    "conclusion": "15字以内的洞察结论",
    "analysis": "洞察开头句。依据掌纹特征的解释。延伸到生活的描述。暗示句。",
    "blind_spots": "生活场景盲区1 · 生活场景盲区2 · 生活场景盲区3",
    "suggestions": "可执行行动1 · 可执行行动2 · 可执行行动3",
    "reminder": "一句带玄学意味的落地金句"
  }},
  ...
  "overall_advice": {{
    "decision": "针对此人掌纹特征的决策建议，带一点玄学收尾",
    "relationship": "针对此人掌纹特征的关系建议，带一点玄学收尾",
    "wealth": "针对此人掌纹特征的财富建议，带一点玄学收尾"
  }}
}}
